...
def extract_cash_transactions(text: str) -> Dict[str, List[float]]:
    """Extract all cash deposits and withdrawals from PDF text."""
    ...
    for i, line in enumerate(lines):
        ...
        elif "CASH WITHDRAW" in line:
            # Look for amount in the same line or nearby lines
            amount = extract_amount_from_line(line)
            if amount:
                result['WITHDRAW'].append(amount)
            else:
                # Check nearby lines for amount
                for j in range(max(0, i-2), min(len(lines), i+3)):
                    if j != i:
...
def extract_amount_from_line(line: str) -> Optional[float]:
    ...
    try:
        ...
        if matches:
            ...
            if abs(amount_float) > 10:  # Filter out small amounts that are likely not transaction amounts
                return amount_float
