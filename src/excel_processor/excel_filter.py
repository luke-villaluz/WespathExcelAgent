...
def extract_cash_transactions_by_series(data: Dict[str, Any]) -> Dict[str, Dict[str, List[float]]]:
    ...
    for i, row in enumerate(rows):
        ...
        cash_account_name = row[3] if len(row) > 3 else ''  # Column D: Cash Account Name
        transaction_type = row[20] if len(row) > 20 else ''  # Column U: Transaction Type
        transaction_amount = row[47] if len(row) > 47 else None  # Column AS: Transaction Amount Local
        
        # Debug output
        print(f"Row {i+1}: Account='{cash_account_name}', Type='{transaction_type}', Amount={transaction_amount}")
        
        # Determine series based on account name (normalize spaces)
        series = None
        normalized_account = ' '.join(str(cash_account_name).split())  # Remove extra spaces
