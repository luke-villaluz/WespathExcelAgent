...
def test_pdf_filter_cash_deposits():
    ...
    assert (
        -53977.40 in transactions["P"]["W"]
    ), f"Should find withdrawal amount -53977.40 in Series P, found {transactions['P']['W']}"
    assert (
        914072.50 in transactions["I"]["D"]
    ), f"Should find deposit amount 914072.50 in Series I, found {transactions['I']['D']}"
