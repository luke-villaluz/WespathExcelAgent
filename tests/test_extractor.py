...
def test_extractor_finds_pdf_files():
    ...
    assert len(pdf_files) > 0, "No PDF files found in Wespath Spreadsheet Files directory"
...
def test_extractor_extracts_text():
    ...
    assert "CASH DEPOSIT" in text or "CASH WITHDRAW" in text, "Should contain cash transactions"