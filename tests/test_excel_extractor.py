...
def test_excel_extractor_finds_excel_files():
    ...
    assert len(excel_files) > 0, "No Excel files found in Wespath Spreadsheet Files directory"
...
def test_excel_extractor_extracts_data():
    ...
    assert len(data['data']) >= 7, "Should have at least 7 rows of data (header + 6 data rows)"
